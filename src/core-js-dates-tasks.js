...
function getTime(date) {
  ...
  const h = getLeadingZero(date.getHours());
  const m = getLeadingZero(date.getMinutes());
  const s = getLeadingZero(date.getSeconds());
  return `${h}:${m}:${s}`;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const subtractionDate = Date.parse(dateEnd) - Date.parse(dateStart);
  return subtractionDate / (24 * 60 * 60 * 1000) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const numberDaysInMonth =
    (nextMonth - new Date(year, month - 1, 1)) / 86400000;
  let counter = 0;
  for (let i = 1; i <= numberDaysInMonth; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  let dayOfTheWeekStartYear = startYear.getDay();
  if (startYear.getDay() === 0) {
    dayOfTheWeekStartYear = 7;
  }
  let dayOfTheWeekCurrDate = date.getDay();
  if (date.getDay() === 0) {
    dayOfTheWeekCurrDate = 7;
  }
  const numberDaysOfYear = (date - startYear) / 86400000 + 1;
  const weekNumber = Math.ceil(numberDaysOfYear / 7);
  return dayOfTheWeekCurrDate < dayOfTheWeekStartYear
    ? weekNumber + 1
    : weekNumber;
}
...
function getNextFridayThe13th(date) {
  let checkDate;
  for (let i = date.getMonth(); i < 30; i += 1) {
    checkDate = new Date(date.getFullYear(), i, 13);
